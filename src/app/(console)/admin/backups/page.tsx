...
export default async function BackupsPage({ searchParams }: {
  ...
  return (
    <>
      ...
      {!configured && (
        <p className="adm-err">
          {t("No backup storage is configured, so nothing is being backed up. The service needs BACKUP_BUCKET, BACKUP_ACCESS_KEY_ID, BACKUP_SECRET_ACCESS_KEY and BACKUP_ENDPOINT.")}
        </p>
      )}

      <div className="adm-cards">
        <div className="adm-card">
          <b>{ago(last?.finished_at ?? null)}</b><span>{t("Last good backup")}</span>
        </div>
        <div className="adm-card"><b>{last?.row_count ?? '—'}</b><span>{t("Rows in it")}</span></div>
        <div className="adm-card"><b>{bytes(last?.bytes ?? null)}</b><span>{t("Stored size")}</span></div>
        <div className="adm-card">
          <b>{runs.filter((r) => r.status === 'failed').length}</b><span>{t("Recent failures")}</span>
        </div>
      </div>
      ...
      {user.role === 'owner' && (
        <form action={backUpNow} style={{ marginBottom: 22 }}>
          <button type="submit" className="adm-btn adm-backup-now">{t("Back up now")}</button>
        </form>
      )}
      ...
      <div className="adm-panel">
        <table className="adm-t">
          <thead>
            <tr><th>{t("Started")}</th><th>{t("Result")}</th><th>{t("Rows")}</th><th>{t("Tables")}</th>
                <th>{t("Size")}</th><th>{t("Stored as")}</th></tr>
          </thead>
          <tbody>
            {runs.length === 0 && (
              <tr><td colSpan={6}><span className="adm-empty">{t("No backup has run yet.")}</span></td></tr>
            )}
            {runs.map((r) => (
              <tr key={r.id}>
      ...
      <div className="adm-panel adm-pad" style={{ marginTop: 20 }}>
        <h2>{t("How to restore one")}</h2>
        <p className="adm-sub">
          Restoring is deliberately not a button here. It replaces every row in the database, and that is not a decision anybody should be one mis-click away from while looking for something else. It is done at a terminal, by someone who means it:
        </p>
        <ol className="adm-sub" style={{ paddingInlineStart: '1.2em' }}>
          <li>{t("Create an empty database.")}</li>
          <li>Run <code>db/migrations/*.sql</code> against it in order. Those files alone rebuild the whole schema — the backup test asserts it on every run, so this cannot quietly stop being true.</li>
        ...
        <p className="adm-sub" style={{ marginBottom: 0 }}>
          {t("The backup holds data only, not schema, so there is one source of truth for the shape of the database and it is the migrations.")}
        </p>
      </div>
    </>
  );
