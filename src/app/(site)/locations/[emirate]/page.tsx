    ...
    <div className="section">
      ...
      <div className="wrap">
        ...
        <div className="loc-cta">
          <Link href={`/quote?type=bulk`} className="btn btn-primary">Request pricing for {l.name}</Link>
        </div>
        ...
        <nav className="others" aria-label="Other emirates">
          ...
          <div className="emirates">
            {locations.filter((x) => x.slug !== l.slug).map((x) => (
              <Link key={x.slug} href={`/locations/${x.slug}`} className="em">{x.name}</Link>
            ))}
          </div>
        </nav>
      </div>
      ...
      <style>{`
        ...
        .notes { display: grid; gap: .85rem; margin: 0; padding-left: 1.15rem; max-width: 68ch; }
